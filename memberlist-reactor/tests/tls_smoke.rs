...
/// the test cert's SAN).
async fn make(id: &str, tls: TlsOptions) -> Memberlist<SmolStr, SocketAddr, TokioRuntime> {
  Memberlist::<SmolStr, _, TokioRuntime>::tls(
    &SocketAddrResolver,
    SmolStr::new(id),
...
async fn zero_close_timeout_is_rejected() {
  ...
  let res = Memberlist::<SmolStr, _, TokioRuntime>::tls(
    &SocketAddrResolver,
  ...
  let ok = Memberlist::<SmolStr, _, TokioRuntime>::tls(
    &SocketAddrResolver,
...
async fn join_without_sni_fails_bounded() {
  ...
  let b = Memberlist::<SmolStr, _, TokioRuntime>::tls(
    &SocketAddrResolver,
    SmolStr::new("b"),
    MaybeResolved::Resolved("127.0.0.1:0".parse::<SocketAddr>().unwrap()),
    Options::new(),
...
async fn send_many_reliable_partial_pre_connect_failure_reports_err() {
  ...
  let sender = Memberlist::<SmolStr, _, TokioRuntime>::tls(
    &SocketAddrResolver,
...
async fn tls_send_reliable_delivers_to_delegate() {
  ...
  let seed = Memberlist::<SmolStr, _, TokioRuntime>::tls(
    &SocketAddrResolver,
