...
struct TcpPipe {
  ...
  inbound: VecDeque<(Instant, bytes::Bytes)>,
  ...
  half_close_after_first: bool,
  /// `true` to NEVER deliver the read==0 FIN anchor on this pipe even when a
  /// FIN is armed: models a peer that half-closed its TLS send direction
  /// (`close_notify`, carried in-band as application ciphertext) but whose
  /// TCP write-side FIN is deferred — so the reader must act on the buffered,
  /// already-decrypted request on the SAME tick it is delivered, leaning only
  /// on the in-band `close_notify`, not on a later TCP read==0. The
  /// small-coalesced post-promotion drain regression knob.
  withhold_fin: bool,
...
impl TcpPipe {
  ...
  fn arm_fin(&mut self, now: Instant, stride: Duration) {
