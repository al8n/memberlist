...
pub struct Options {
  ...
  /// the free sockets having already been spent on the oldest of them. A burst past
  /// that is shed newest-first, and
  /// [`send_reliable`](crate::Memberlist::send_reliable) refuses at the call site as
  /// backpressure before an exchange is built. The engine's field docs carry the
  /// full contract.
  ///
  /// Must be non-zero. Defaults to
