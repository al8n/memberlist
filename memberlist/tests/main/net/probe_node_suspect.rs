...
macro_rules! probe_node_suspect {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake $kind:snake _net_probe_node_suspect >]() {
...
mod tokio {
  ...
  #[cfg(feature = "tls")]
  probe_node_suspect!(tokio("tls", memberlist_net::tests::tls_stream_layer::<TokioRuntime>().await));

  #[cfg(feature = "native-tls")]
  probe_node_suspect!(tokio("native-tls", memberlist_net::tests::native_tls_stream_layer::<TokioRuntime>().await));
}
...
mod async_std {
  ...
  #[cfg(feature = "tls")]
  probe_node_suspect!(async_std("tls", memberlist_net::tests::tls_stream_layer::<AsyncStdRuntime>().await));

  #[cfg(feature = "native-tls")]
  probe_node_suspect!(async_std("native-tls", memberlist_net::tests::native_tls_stream_layer::<AsyncStdRuntime>().await));
}
...
mod smol {
  ...
  #[cfg(feature = "tls")]
  probe_node_suspect!(smol("tls", memberlist_net::tests::tls_stream_layer::<SmolRuntime>().await));

  #[cfg(feature = "native-tls")]
  probe_node_suspect!(smol("native-tls", memberlist_net::tests::native_tls_stream_layer::<SmolRuntime>().await));
}
