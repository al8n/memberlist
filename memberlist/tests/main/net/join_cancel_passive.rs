...
macro_rules! join_cancel_passive {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_join_cancel_passive >]() {
...
mod tokio {
  ...
  join_cancel_passive!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  join_cancel_passive!(tokio(
    "tls",
...
mod async_std {
  ...
  join_cancel_passive!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  join_cancel_passive!(async_std(
    "tls",
...
mod smol {
  ...
  join_cancel_passive!(smol("tcp", Tcp::<SmolRuntime>::new()));

  #[cfg(feature = "tls")]
  join_cancel_passive!(smol(
    "tls",
