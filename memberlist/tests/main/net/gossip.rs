...
macro_rules! gossip {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_gossip >]() {
...
mod tokio {
  ...
  gossip!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  gossip!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));
...
mod smol {
  ...
  gossip!(smol("tcp", Tcp::<SmolRuntime>::new()));
