    ...
    paste::paste! {
      ...
      fn [< test_ $rt:snake _ $kind:snake _probe >]() {
        [< $rt:snake _run >](async move {
          ...
          probe(t1, Options::lan(), t2, Options::lan()).await;
        });
      }
      ...
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _probe_with_checksum >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = NetTransportOptions::<SmolStr, _, $layer<[< $rt:camel Runtime >]>>::with_stream_layer_options("probe_node_1".into(), $expr);
          t1_opts.add_bind_address(next_socket_addr_v4(0));
          ...
          t2_opts.add_bind_address(next_socket_addr_v4(0));
          let t2 = NetTransport::new(t2_opts).await.unwrap();
          probe(t1, Options::lan().with_checksum_algo(Some(Default::default())), t2, Options::lan().with_checksum_algo(Some(Default::default()))).await;
        });
      }
      ...
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _probe_with_all >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = NetTransportOptions::<SmolStr, _, $layer<[< $rt:camel Runtime >]>>::with_stream_layer_options("probe_node_1".into(), $expr);
          t1_opts.add_bind_address(next_socket_addr_v4(0));
