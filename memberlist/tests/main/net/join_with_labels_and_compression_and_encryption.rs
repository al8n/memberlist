...
use memberlist_net::{compression::Compressor, security::SecretKey, Label};
      ...
      fn [< test_ $rt:snake _ $kind:snake _net_join_with_labels >]() {
        [< $rt:snake _run >](async move {
          memberlist_join_with_labels_and_compression_and_encryption(|idx, label, compressor, pk| async move {
            let mut t1_opts = NetTransportOptions::<SmolStr, _>::new(format!("join_with_labels_and_compression_and_encryption_node_{idx}").into())
              .with_label(label)
...
mod tokio {
  ...
  join_with_labels_and_compression_and_encryption!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  join_with_labels_and_compression_and_encryption!(tokio(
    "tls",
...
mod async_std {
  ...
  join_with_labels_and_compression_and_encryption!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  join_with_labels_and_compression_and_encryption!(async_std(
    "tls",
...
mod smol {
  ...
  join_with_labels_and_compression_and_encryption!(smol("tcp", Tcp::<SmolRuntime>::new()));

  #[cfg(feature = "tls")]
  join_with_labels_and_compression_and_encryption!(smol(
    "tls",
