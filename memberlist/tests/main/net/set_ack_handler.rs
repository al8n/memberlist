...
macro_rules! set_ack_handler {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_set_ack_handler >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = NetTransportOptions::<SmolStr, _>::new("set_ack_handler_node_1".into());
...
mod tokio {
  ...
  set_ack_handler!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  set_ack_handler!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));
...
mod smol {
  ...
  set_ack_handler!(smol("tcp", Tcp::<SmolRuntime>::new()));
