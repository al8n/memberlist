...
macro_rules! join_cancel {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_join_cancel >]() {
...
mod tokio {
  ...
  join_cancel!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  join_cancel!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  join_cancel!(async_std(
    "tls",
...
mod smol {
  ...
  join_cancel!(smol("tcp", Tcp::<SmolRuntime>::new()));
