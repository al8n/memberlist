...
macro_rules! send {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_send >]() {
...
mod tokio {
  ...
  send!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  send!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));
...
mod smol {
  ...
  send!(smol("tcp", Tcp::<SmolRuntime>::new()));
