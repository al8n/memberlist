...
macro_rules! dead_node_left {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_dead_node_left >]() {
...
mod tokio {
  ...
  dead_node_left!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  dead_node_left!(tokio(
    "tls",
...
mod async_std {
  ...
  dead_node_left!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  dead_node_left!(async_std(
    "tls",
...
mod smol {
  ...
  dead_node_left!(smol("tcp", Tcp::<SmolRuntime>::new()));
