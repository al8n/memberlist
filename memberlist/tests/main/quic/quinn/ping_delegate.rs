...
macro_rules! ping_delegate {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_ping_delegate >]() {
...
mod tokio {
  ...
  ping_delegate!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  ping_delegate!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  ping_delegate!(async_std(
    "tls",
...
mod smol {
  ...
  ping_delegate!(smol("tcp", Tcp::<SmolRuntime>::new()));
