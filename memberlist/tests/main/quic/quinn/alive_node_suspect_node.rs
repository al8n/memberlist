...
macro_rules! alive_node_suspect_node {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_alive_node_suspect_node >]() {
...
mod tokio {
  ...
  alive_node_suspect_node!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  alive_node_suspect_node!(tokio(
    "tls",
...
mod async_std {
  ...
  alive_node_suspect_node!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  alive_node_suspect_node!(async_std(
    "tls",
...
mod smol {
  ...
  alive_node_suspect_node!(smol("tcp", Tcp::<SmolRuntime>::new()));

  #[cfg(feature = "tls")]
  alive_node_suspect_node!(smol(
    "tls",
