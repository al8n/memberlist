...
mod tokio {
  ...
  merge_state!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  merge_state!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  merge_state!(async_std(
    "tls",
...
mod smol {
  ...
  merge_state!(smol("tcp", Tcp::<SmolRuntime>::new()));
