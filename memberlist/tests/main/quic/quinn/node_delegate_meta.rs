...
macro_rules! node_delegate_meta {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_node_delegate_meta >]() {
...
mod tokio {
  ...
  node_delegate_meta!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  node_delegate_meta!(tokio(
    "tls",
...
mod async_std {
  ...
  node_delegate_meta!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  node_delegate_meta!(async_std(
    "tls",
...
mod smol {
  ...
  node_delegate_meta!(smol("tcp", Tcp::<SmolRuntime>::new()));

  #[cfg(feature = "tls")]
  node_delegate_meta!(smol(
    "tls",
