...
macro_rules! suspect_node {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_suspect_node >]() {
...
mod tokio {
  ...
  suspect_node!(tokio("tcp", Tcp::<TokioRuntime>::new()));
...
mod async_std {
  ...
  suspect_node!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  suspect_node!(async_std(
    "tls",
...
mod smol {
  ...
  suspect_node!(smol("tcp", Tcp::<SmolRuntime>::new()));
