...
macro_rules! create_shutdown {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_create_shutdown >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = NetTransportOptions::<SmolStr, _>::new("create_shutdown_node_1".into());
...
mod tokio {
  ...
  create_shutdown!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  create_shutdown!(tokio(
    "tls",
...
mod async_std {
  ...
  create_shutdown!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  create_shutdown!(async_std(
    "tls",
...
mod smol {
  ...
  create_shutdown!(smol("tcp", Tcp::<SmolRuntime>::new()));

  #[cfg(feature = "tls")]
  create_shutdown!(smol(
    "tls",
