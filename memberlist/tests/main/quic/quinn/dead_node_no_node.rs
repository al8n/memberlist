...
macro_rules! dead_node_no_node {
  ($rt: ident ($kind:literal, $expr: expr)) => {
    paste::paste! {
      #[test]
      fn [< test_ $rt:snake _ $kind:snake _net_dead_node_no_node >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = NetTransportOptions::<SmolStr, _>::new("dead_node_no_node_1".into());
...
mod tokio {
  ...
  dead_node_no_node!(tokio("tcp", Tcp::<TokioRuntime>::new()));

  #[cfg(feature = "tls")]
  dead_node_no_node!(tokio(
    "tls",
...
mod async_std {
  ...
  dead_node_no_node!(async_std("tcp", Tcp::<AsyncStdRuntime>::new()));

  #[cfg(feature = "tls")]
  dead_node_no_node!(async_std(
    "tls",
...
mod smol {
  ...
  dead_node_no_node!(smol("tcp", Tcp::<SmolRuntime>::new()));

  #[cfg(feature = "tls")]
  dead_node_no_node!(smol(
    "tls",
