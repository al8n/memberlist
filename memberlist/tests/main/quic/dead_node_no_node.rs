    ...
    paste::paste! {
      ...
      fn [< test_ $rt:snake _ $kind:snake _dead_node_no_node >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = QuicTransportOptions::<SmolStr, _, $layer<[< $rt:camel Runtime >]>>::with_stream_layer("dead_node_no_node_1".into());
      ...
      fn [< test_ $rt:snake _ $kind:snake _dead_node_no_node_with_compression >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = QuicTransportOptions::<SmolStr, _, $layer<[< $rt:camel Runtime >]>>::with_stream_layer("dead_node_no_node_1".into()).with_compressor(Some(Default::default())).with_offload_size(10);
          t1_opts.add_bind_address(next_socket_addr_v4(0));
