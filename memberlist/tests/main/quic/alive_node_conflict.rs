    ...
    paste::paste! {
      ...
      fn [< test_ $rt:snake _ $kind:snake _alive_node_conflict >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = QuicTransportOptions::<SmolStr, _, $layer<[< $rt:camel Runtime >]>>::with_stream_layer_options("alive_node_conflict_1".into(), $expr);
      ...
      fn [< test_ $rt:snake _ $kind:snake _alive_node_conflict_with_compression >]() {
        [< $rt:snake _run >](async move {
          let mut t1_opts = QuicTransportOptions::<SmolStr, _, $layer<[< $rt:camel Runtime >]>>::with_stream_layer_options("alive_node_conflict_1".into(), $expr).with_compressor(Some(Default::default())).with_offload_size(10);
          t1_opts.add_bind_address(next_socket_addr_v4(0));
