...
use crate::{
  ...
  error::Error,
  proto::{Ack, Alive, Dead, Message, Nack, PushNodeState, State, Suspect},
  ...
  A: CheapClone + Send + Sync + 'static,
{
  ...
  async fn notify_join(&self, node: Arc<crate::proto::NodeState<Self::Id, Self::Address>>) {
  ...
  async fn notify_leave(&self, node: Arc<crate::proto::NodeState<Self::Id, Self::Address>>) {
    let mu = self.enabled.lock().await;
    if *mu {
  ...
  async fn notify_update(&self, node: Arc<crate::proto::NodeState<Self::Id, Self::Address>>) {
    let mu = self.enabled.lock().await;
    if *mu {
