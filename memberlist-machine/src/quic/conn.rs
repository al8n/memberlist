...
impl ConnEntry {
    pub(crate) fn conn_mut(&mut self) -> &mut Connection {
        ...
        if !self.established_at_least_once
...
impl ConnTable {
    ...
    pub(crate) fn insert_accepted(
        ...
        assert_eq!(slot, ch.0, "accepted connection slab slot must equal ConnectionHandle");
    ...
    pub(crate) fn iter_handles(&self) -> Vec<ConnectionHandle> {
        self.conns.iter().map(|(k, _)| ConnectionHandle(k)).collect()
    }
}
...
mod tests {
    ...
    fn quinn_pair() -> (QuinnEndpoint, QuinnEndpoint, super::super::crypto::QuicConfig) {
    ...
    fn reap_if_drained_is_false_until_drained_then_removes() {
        ...
        let ch = t
            .get_or_dial(&mut client, now, cfg.client().clone(), peer, "localhost")
            .unwrap();
        assert!(!t.reap_if_drained(&mut client, ch), "fresh conn is not drained");
    ...
    fn get_or_dial_redials_when_cached_conn_is_closed_not_drained() {
        ...
        assert!(t.get(ch_redial).is_some(), "new slab entry inserted by the redial");
    ...
    fn reap_if_drained_handle_equality_guard_idempotent_when_peers_absent() {
        ...
        assert_eq!(t.handle_for(&peer), None, "peers state unchanged (no resurrection)");
    }
}
