...
use memberlist_wire::typed::{DelegateVersion, ProtocolVersion};
...
impl crate::delegate::AliveDelegate<SmolStr, SocketAddr> for RejectAllAlive {
  fn notify_alive(&self, _peer: &memberlist_wire::typed::NodeState<SmolStr, SocketAddr>) -> bool {
    false
  }
}
  ...
  match tx {
    Transmit::Compound(cmp) => {
      let anys: Vec<_> = cmp
        ...
        .collect();
      memberlist_wire::framing::encode_compound(&anys)
...
fn gossip_lone_near_mtu_membership_broadcast_emits_packet() {
  ...
  let (id, av, frame_len) = loop {
    ...
    .with_protocol_version(ProtocolVersion::V1)
    .with_delegate_version(DelegateVersion::V1);
    let any = memberlist_wire::message_to_any(&Message::Alive(av.clone())).expect("bridge");
    let len = memberlist_wire::framing::encode_message(&any)
      .expect("encode")
      .len();
