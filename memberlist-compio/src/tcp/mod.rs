  ...
  A: Clone + Send + 'static,
{
    ...
    AR: AdvertiseAddrResolver,
  {
    ...
    // socket and the TCP reliable listener MUST share a port. Claiming a port
    // both protocols will accept — and getting past the ones only one of them
    // will — is the shared helper's job.
