//! As the decision made moby, this file does not accept new additions. Ported into this
//! project for try to give a random valid name to the node. But I highly recommend to
//! give a name to the node by yourself.
//!
//! This file is officially "frozen" - no new additions will be accepted.
//!
...
pub(super) fn random_name() -> String {
  ...
  loop {
    let adj = LEFT[rng.gen_range(0..LEFT.len())].to_string();
    let surname = RIGHT[rng.gen_range(0..RIGHT.len())].to_string();

    if adj == "boring" && surname == "wozniak" {
      continue;
    }
    let mut suffix = [0; 3];
    rng.fill(&mut suffix[..]);
    break format!(
      "{}-{}-{}{}{}",
      adj, surname, suffix[0], suffix[1], suffix[2]
    );
  }
}
