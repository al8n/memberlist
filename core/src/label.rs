...
pub fn add_label_header_to_packet(src: &[u8], label: &[u8]) -> Result<Bytes, Error> {
  if !label.is_empty() {
...
pub fn remove_label_header_from_packet(mut buf: Bytes) -> Result<(Bytes, Bytes), Error> {
  ...
  fn remove_test_cases() -> Vec<(&'static str, TestCase)> {
    vec![
      ...
      (
        "error with no label",
        TestCase {
          buf: build_test_data(MessageType::Err as u8, b"blah"),
          expect_label: Bytes::new(),
          expect_packet: build_test_data(MessageType::Err as u8, b"blah"),
          expect_err: None,
        },
      ),
      (
