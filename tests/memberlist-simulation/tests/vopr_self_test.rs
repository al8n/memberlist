...
fn inline_monotonicity_checker_catches_a_regression() {
  ...
  fn check_seq(incs: &[u32]) -> CheckResult {
    ...
    for &i in incs {
      if i < max {
        return CheckResult::violation("incarnation regressed");
      }
      max = max.max(i);
    }
    CheckResult::Ok
  ...
  assert!(
    check_seq(&[1, 5, 3]).is_violation(),
    "the checker must bite on a regression"
  );
}
...
fn incarnation_monotonic_checker_bites_on_regression() {
  ...
  assert!(
    ck.observe_one(obs, &sub, Some(3), Some(State::Alive), false)
      .is_violation(),
...
fn incarnation_monotonic_checker_allows_prune_rejoin() {
  ...
  assert!(
    ck.observe_one(obs, &sub, Some(5), Some(State::Dead), false)
      .is_ok()
  );
  assert!(
    ck.observe_one(obs, &sub, Some(1), Some(State::Alive), true)
      .is_ok(),
  ...
  assert!(
    ck2
      .observe_one(obs, &sub, Some(5), Some(State::Dead), false)
      .is_ok()
  );
  assert!(
    ck2
      .observe_one(obs, &sub, Some(1), Some(State::Alive), false)
      .is_violation(),
...
fn no_resurrection_checker_bites_without_prune() {
  ...
  assert!(
    ck.observe_one(obs, &sub, Some(State::Alive), Some(5), false)
      .is_violation(),
...
fn no_resurrection_checker_allows_realive_after_prune() {
  ...
  assert!(
    ck.observe_one(obs, &sub, Some(State::Alive), Some(6), false)
      .is_ok()
  );
  ...
  assert!(
    ck2
      .observe_one(obs, &sub, Some(State::Left), Some(1), false)
      .is_ok()
  );
  assert!(
    ck2
      .observe_one(obs, &sub, Some(State::Alive), Some(1), true)
      .is_ok(),
...
fn meta_per_incarnation_checker_bites_on_divergent_meta() {
  ...
  assert!(
    ck.observe_one(&sub, 3, (vec![9, 9, 9], 0, 0))
      .is_violation(),
...
fn self_incarnation_checker_bites_on_decrease() {
  ...
  assert!(
    ck.observe_one(h, 3).is_violation(),
    "self-incarnation 5->3 must fire"
  );
}
...
fn self_liveness_checker_bites_on_self_dead_while_running() {
  ...
  assert!(
    ck.observe_one(h, true, Some(State::Dead)).is_violation(),
...
fn self_liveness_checker_allows_self_left_when_not_running() {
  ...
  let h = addr(1);
  assert!(
    ck.observe_one(h, false, Some(State::Left)).is_ok(),
...
fn illegal_pair_checker_bites_on_dead_to_suspect_same_incarnation() {
  ...
  assert!(
    ck.observe_one(obs, &sub, Some(State::Suspect), Some(5), false)
      .is_violation(),
...
fn boundedness_checker_bites_on_excess_members() {
  ...
  assert!(
    ck.observe_one(h, 6, 5, 0).is_violation(),
...
fn convergence_checker_passes_on_settled_cluster() {
  ...
  c.inject_alive(a0, "n2".into(), a2, 1);
  c.inject_alive(a1, "n0".into(), a0, 1);
  c.inject_alive(a1, "n2".into(), a2, 1);
  c.inject_alive(a2, "n0".into(), a0, 1);
  c.inject_alive(a2, "n1".into(), a1, 1);
  for _ in 0..400 {
    c.step();
  ...
  let conv = ConvergenceChecker::new();
  assert!(
    conv.check(&c, &HashSet::new()).is_ok(),
    "a settled all-alive mesh has converged"
  );
}
...
fn convergence_checker_bites_on_departed_node_seen_alive() {
  ...
  let conv = ConvergenceChecker::new();
  assert!(
    conv.check(&c, &left).is_violation(),
...
fn checker_clear_observer_resets_baselines_for_restarted_node() {
  ...
  use memberlist_simulation::checker::{
    IllegalPairChecker, IncarnationMonotonicChecker, NoResurrectionChecker,
  };
  let obs = addr(1);
  let sub: SmolStr = "b".into();
  ...
  assert!(
    s1.observe_one(obs, &sub, Some(5), Some(State::Dead), false)
      .is_ok()
  );
  s1.clear_observer(obs);
  assert!(
    s1.observe_one(obs, &sub, Some(1), Some(State::Alive), false)
      .is_ok(),
  ...
  assert!(
    s2.observe_one(obs, &sub, Some(State::Dead), Some(5), false)
      .is_ok()
  );
  s2.clear_observer(obs);
  assert!(
    s2.observe_one(obs, &sub, Some(State::Alive), Some(5), false)
      .is_ok(),
  ...
  assert!(
    s6.observe_one(obs, &sub, Some(State::Dead), Some(5), false)
      .is_ok()
  );
  s6.clear_observer(obs);
  assert!(
    s6.observe_one(obs, &sub, Some(State::Suspect), Some(5), false)
      .is_ok(),
    "post-restart observer should not fire on a stale Dead baseline"
  );
}
...
fn no_resurrection_checker_retains_baseline_through_unpruned_absence() {
  ...
  assert!(
    ck.observe_one(obs, &sub, None, None, false).is_ok(),
  ...
  assert!(
    ck2.observe_one(obs, &sub, None, None, true).is_ok(),
    "a prune clears the baseline"
  );
  assert!(
    ck2
      .observe_one(obs, &sub, Some(State::Alive), Some(5), false)
      .is_ok(),
...
fn convergence_checker_bites_on_wrong_address_binding() {
  ...
  assert!(
    r.reason()
      .is_some_and(|s| s.contains("binds") && s.contains("n1")),
...
fn incarnation_monotonic_clears_terminal_baseline_on_prune() {
  ...
  assert!(
    ck.observe_one(obs, &sub, None, None, true).is_ok(),
...
fn convergence_rejects_a_crashed_node_still_seen_alive() {
  ...
  assert!(
    r.reason()
      .is_some_and(|s| s.contains("crashed") && s.contains("n2")),
