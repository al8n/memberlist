...
//! byte pipe.
//!
//! The plain-TCP reliable path is [`crate::streams::Labeled`] wrapping
//! [`crate::streams::Passthrough`]: a one-time `[LABELED_TAG=12][len][label]`
...
//! passthrough) lives in [`crate::streams::label`]; the byte pipe and the
//! decorator live in [`crate::streams::labeled`]. TLS's in-band `close_notify`
...
/// pure byte pipe.
///
/// Carried by a [`crate::streams::StreamEndpoint`] parameterised with
