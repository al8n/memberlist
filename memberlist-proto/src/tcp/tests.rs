...
fn policy_failed_bridge_rejects_ingress_before_reap() {
  ...
  let new_opts = EncryptionOptions::new().with_keyring(Keyring::new(SecretKey::Aes256([0x42; 32])));
  ...
  let unit = encode_reliable_unit_with_encryption(&CompressionOptions::new(), &new_opts, &response)
