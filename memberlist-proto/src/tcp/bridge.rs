...
/// Encode `msg` as one reliable unit (disabled compression/encryption → the
/// unit payload is the framed message verbatim), ready to feed straight
/// through a promoted bridge's established intake via `handle_transport_data`.
fn user_data_unit(payload: &[u8]) -> Vec<u8> {
  let msg = crate::typed::Message::<SmolStr, SocketAddr>::UserData(Bytes::copy_from_slice(payload));
  let framed = crate::wire::encode_message::<SmolStr, SocketAddr>(&msg).expect("encode user data");
  crate::encode_reliable_unit(&crate::CompressionOptions::new(), &framed)
}

/// `fail_connection_lost` on an `Active` bridge — a mid-exchange transport
/// RESET / I/O error reported by the driver AFTER a complete frame has
/// dispatched but BEFORE the peer's clean recv-half EOF — transitions to
/// `Failed(ConnectionLost)` AND discards the FSM's pre-FIN queued events
/// (`fail`'s `phase ∉ RecvClosed/BothClosed` hygiene). The subsequent terminal
/// reap therefore surfaces NO `Event::UserPacket`: the dispatched frame was
/// never authorized by a clean close, and a transport failure does not
/// authorize it either. The reap's lifecycle notice routes through the
/// `BridgeFailure::ConnectionLost` reason-string arm.
#[test]
fn fail_connection_lost_discards_unauthorized_frame_then_reaps_errored() {
  let now = Instant::now();
  let (mut server, mut ep_s) = promoted_inbound(now, 7800);

  // Dispatch a complete one-way UserData frame into the inbound stream. The
  // FSM queues `UserDataReceived`, but the bridge stays `Active` — no recv
  // FIN has been observed, so the side effect is held pending authorization.
  let unit = user_data_unit(b"unauthorized");
  server
    .handle_transport_data(&unit, now)
    .expect("a complete UserData unit is accepted");
  assert!(
    matches!(
      server.phase_ref(),
      BridgePhase::Established(LinkState::Active)
    ),
    "the bridge is still Active before any FIN, got {:?}",
    phase_label(server.phase_ref())
  );

  // The driver reports the connection dropped before the clean EOF.
  ...
  // The terminal reap drains the (now-cleared) event queue and builds the
  // lifecycle notice through the ConnectionLost reason-string arm. NO
  // UserPacket may surface — the frame was never authorized.
  server.drain_then_reap(&mut ep_s, now);
  let mut saw_user_packet = false;
  while let Some(ev) = ep_s.poll_event() {
    if matches!(ev, Event::UserPacket(_)) {
      saw_user_packet = true;
    }
  }
  assert!(
    !saw_user_packet,
    "a connection lost BEFORE the recv-half EOF must not commit the \
       dispatched frame — no UserPacket may leak"
  );
}

/// First failure wins: a `fail_connection_lost` following a prior
/// `Failed(Decode)` does NOT overwrite the original cause — `fail`'s
/// leading `Failed(_) => return` guard makes the transition sticky.
#[test]
fn fail_connection_lost_is_sticky_over_prior_failure() {
  let now = Instant::now();
  let (mut server, _ep_s) = promoted_inbound(now, 7810);

  // A mid-frame read == 0 fails the bridge Decode first.
  let res = server.handle_transport_data(&[], now);
  assert!(res.is_err());
  assert!(matches!(
    server.phase_ref(),
...
/// `fail_dial_retired` terminalizes a `Handshaking` dialer whose dial intent
/// was retired by the inner endpoint before a `Stream` could be minted: the
/// bridge enters `Failed(DialRetired)` with NO `Stream`, and `drain_then_reap`
/// on a no-`Stream` bridge is a clean no-op (no FSM lifecycle notice is owed).
#[test]
fn fail_dial_retired_pre_stream_terminalizes_and_reaps_cleanly() {
  let now = Instant::now();
  // A fresh, un-promoted dialer is `Handshaking` with no `Stream`.
  let dialer = RawRecords::dialer(label("cluster-x"), false);
  let mut client = StreamBridge::<SmolStr, SocketAddr, RawRecords>::new(
    dialer,
    now + Duration::from_secs(10),
  ...
  assert!(client.stream_is_none(), "a fresh dialer has no Stream yet");
  ...
  assert!(
    ...
    "a retired dial intent maps to Failed(DialRetired), got {:?}",
    phase_label(client.phase_ref())
  );
  assert!(client.is_terminal());
  assert!(
    client.stream_is_none(),
    "no Stream is minted on a retired dial"
  );

  let mut ep: Endpoint<SmolStr, SocketAddr> =
    Endpoint::new_seeded(EndpointOptions::new(SmolStr::new("cli"), addr(7000)));
  client.drain_then_reap(&mut ep, now);
  assert!(
    ep.poll_event().is_none(),
    "no endpoint events on the no-Stream dial-retired reap path"
  );
}

/// `fail_dial_retired` on a PROMOTED bridge holding a `Stream` builds the
...
fn drain_then_reap_dial_retired_routes_errored_notice() {
  let now = Instant::now();
  let (mut client, _server) = handshaking_pair("cluster-x", now + Duration::from_secs(10));
  // The dialer is never handshaking; promote an outbound user message so a
  // `Stream` is present for the reason-string arm.
  let mut ep_c: Endpoint<SmolStr, SocketAddr> =
...
/// On the INSECURE plain-TCP record layer (`RawRecords::is_secure() == false`)
/// a runtime encryption-policy change fails the live bridge to
/// `Failed(EncryptionPolicyChanged)` — any bytes the FSM already encoded under
/// the prior policy must not reach the wire post-enablement, so the SWIM FSM
/// retries the exchange under a fresh bridge built with the new policy. The
/// reap's lifecycle notice routes through the `EncryptionPolicyChanged`
/// reason-string arm. (The TLS mirror force-disables and stays running, since
/// TLS already protects the reliable path.)
#[test]
fn set_encryption_fails_insecure_bridge_then_reaps_errored() {
  let now = Instant::now();
  let (mut server, mut ep_s) = promoted_inbound(now, 7830);
  assert!(
    matches!(
      server.phase_ref(),
      BridgePhase::Established(LinkState::Active)
    ),
    "the promoted acceptor starts Active"
  );

  // A policy change on plain TCP fails the bridge regardless of whether the
  // new options are enabled or disabled (the cascade is unconditional on an
  // insecure transport).
  server.set_encryption(crate::EncryptionOptions::new());
  assert!(
    matches!(
      server.phase_ref(),
      BridgePhase::Established(LinkState::Failed(BridgeFailure::EncryptionPolicyChanged))
    ),
    "a plain-TCP policy change maps to Failed(EncryptionPolicyChanged), got {:?}",
    phase_label(server.phase_ref())
  );
  assert!(server.is_terminal());
  ...
  assert!(
    ep_s.poll_event().is_none(),
    "an EncryptionPolicyChanged StreamErrored notice maps to no public Event"
  );
}

/// The FIN observers are sticky no-ops once the bridge is terminal: after a
/// `fail_connection_lost`, both `observe_send_fin` (driven via a redundant
/// `pump_out`) and a redundant recv EOF leave the `Failed(ConnectionLost)`
/// phase intact — the failure cause is never overwritten by a half-close.
#[test]
fn fin_observers_are_noops_on_failed_phase() {
  let now = Instant::now();
  let (mut server, _ep_s) = promoted_inbound(now, 7840);
  ...
  // A redundant pump_out (send-half observation) and a redundant empty-slice
  // EOF (recv-half observation) on a terminal bridge are no-ops. Ignoring Err:
  // both are no-ops on a terminal bridge; the sticky phase is asserted below.
  let _ = server.pump_out(now);
  let _ = server.handle_transport_data(&[], now);
  assert!(
    matches!(
      server.phase_ref(),
    ...
    "FIN observers must not overwrite a sticky Failed phase, got {:?}",
    phase_label(server.phase_ref())
  );
}
