...
pub trait Delegate: Send + Sync + 'static {
  ...
  #[cfg(feature = "async")]
  async fn get_broadcasts(&self, overhead: usize, limit: usize) -> Result<Vec<Bytes>, Self::Error>;
  ...
  /// since doing so would block the entire UDP packet receive loop.
  #[cfg(not(feature = "async"))]
  fn get_broadcasts(&self, overhead: usize, limit: usize) -> Result<Vec<Bytes>, Self::Error>;
