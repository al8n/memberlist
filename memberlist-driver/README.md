<div align="center">
<h1>memberlist-driver</h1>
</div>
<div align="center">

Runtime-agnostic glue shared by the memberlist async driver crates — the pieces every
runtime needs and none of them should implement twice.

[<img alt="github" src="https://img.shields.io/badge/github-al8n/memberlist-8da0cb?style=for-the-badge&logo=Github" height="22">][Github-url]
<img alt="LoC" src="https://img.shields.io/endpoint?url=https%3A%2F%2Fgist.githubusercontent.com%2Fal8n%2Fd29ceff54c025fe4e8b144a51efb9324%2Fraw%2Fmemberlist-driver" height="22">
[<img alt="Build" src="https://img.shields.io/github/actions/workflow/status/al8n/memberlist/coverage.yml?logo=Github-Actions&style=for-the-badge" height="22">][CI-url]
[<img alt="codecov" src="https://img.shields.io/codecov/c/gh/al8n/memberlist?style=for-the-badge&token=6R3QFWRWHL&logo=codecov" height="22">][codecov-url]

[<img alt="docs.rs" src="https://img.shields.io/badge/docs.rs-memberlist--driver-66c2a5?style=for-the-badge&labelColor=555555&logo=docs.rs" height="20">][doc-url]
[<img alt="crates.io" src="https://img.shields.io/crates/v/memberlist-driver?style=for-the-badge&logo=rust" height="22">][crates-url]
[<img alt="crates.io" src="https://img.shields.io/crates/d/memberlist-driver?color=critical&logo=rust&style=for-the-badge" height="22">][crates-url]
<img alt="license" src="https://img.shields.io/badge/License-MPL%202.0-blue.svg?style=for-the-badge&fontColor=white&logoColor=ffffff&logo=data:image/svg+xml;base64," height="22">

[<img alt="Discord" src="https://img.shields.io/discord/835936528140206122?style=for-the-badge&logo=discord&logoColor=white&label=Discord&color=7289da" height="22">][discord]

</div>

## Introduction

A memberlist driver binds [`memberlist-proto`]'s Sans-I/O `Endpoint` to a real async
runtime: it owns a run loop, a channel/cell substrate, and delegate dispatch, and drives
the machine's `handle_*` / `poll_*` surface from there. `memberlist-driver` holds the
pieces of that job which have no opinion about *which* runtime is doing the driving, so
[`memberlist-reactor`] (tokio / smol) and [`memberlist-compio`] (compio) share one
implementation instead of maintaining two.

It performs no I/O and spawns nothing. What stays out of it is anything runtime-shaped:
the run loop, the channel and cell substrate, and the delegate dispatch live in each
driver crate.

Most applications never depend on this crate directly — pick a driver, or the [`memberlist`]
umbrella facade, and it comes along.

## What lives here

| Provides | Description |
|----------|-------------|
| `MemberlistSnapshot<I, A>` | the immutable, point-in-time membership view a driver republishes after every change, with the read helpers a handle exposes (`local`, `by_id`, `online_members`, `members_by`, `num_members_by`, `members_map_by`) |
| `error` | the driver error payloads shared across runtimes: `GossipMtuTooSmall`, `InvalidOption`, `JoinFailed` |
| `observation_payload_bytes` | the byte-backstop weight of one event, for the observation channel's application-flood bound: `Some(len)` for the payload-bearing variants, `None` for the small membership/control events the count cap already bounds |

## Installation

```toml
[dependencies]
memberlist-driver = "0.1"
```

The crate is `std`-only and carries no feature flags.

## The family

- [`memberlist`] — the umbrella facade: one dependency, driver chosen by feature.
- [`memberlist-proto`] — the Sans-I/O SWIM state machine and wire codec.
- [`memberlist-reactor`] — the runtime-agnostic async driver (tokio / smol).
- [`memberlist-compio`] — the completion-I/O (io_uring) async driver.
- [`memberlist-embedded`], [`memberlist-smoltcp`], [`memberlist-embassy`] — the bare-metal `no_std` drivers.

## License

`memberlist-driver` is under the terms of the MPL-2.0 license.

See [LICENSE] for details.

Copyright (c) 2025 Al Liu.

[`memberlist`]: https://crates.io/crates/memberlist
[`memberlist-proto`]: https://crates.io/crates/memberlist-proto
[`memberlist-reactor`]: https://crates.io/crates/memberlist-reactor
[`memberlist-compio`]: https://crates.io/crates/memberlist-compio
[`memberlist-embedded`]: https://crates.io/crates/memberlist-embedded
[`memberlist-smoltcp`]: https://crates.io/crates/memberlist-smoltcp
[`memberlist-embassy`]: https://crates.io/crates/memberlist-embassy
[Github-url]: https://github.com/al8n/memberlist/
[CI-url]: https://github.com/al8n/memberlist/actions/workflows/coverage.yml
[codecov-url]: https://app.codecov.io/gh/al8n/memberlist/
[doc-url]: https://docs.rs/memberlist-driver
[crates-url]: https://crates.io/crates/memberlist-driver
[LICENSE]: https://github.com/al8n/memberlist/blob/main/LICENSE
[discord]: https://discord.gg/f3EtWBM7ke
