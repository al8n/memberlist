...
macro_rules! handle_ping_no_label_with_compression_test_suites {
  ($($prefix:literal: )? $layer:ident<$rt:ident>::$run:ident({ $s: expr })) => {
    ...
    $crate::__handle_ping_no_label_with_compression!($($prefix: )? $layer<$rt>::$run({ $s }));
  };
}
    ...
    paste::paste! {
      memberlist_core::unit_tests_with_expr!($run(
        [< $($prefix)? _handle_v4_ping_server_no_label_with_compression_client_no_label_with_compression >] ({
          let s = $s;
          let c = $s;
          if let Err(e) = memberlist_quic::tests::handle_ping::no_label_with_compression::server_no_label_with_compression_client_no_label_with_compression::<$layer<$rt>, $rt>(
            s,
            c,
      ...
      memberlist_core::unit_tests_with_expr!($run(
        [< $($prefix)? _handle_v4_ping_server_no_label_no_compression_client_no_label_with_compression >] ({
          let s = $s;
          let c = $s;
          if let Err(e) = memberlist_quic::tests::handle_ping::no_label_with_compression::server_no_label_no_compression_client_no_label_with_compression::<$layer<$rt>, $rt>(
            s,
            c,
            memberlist_core::transport::tests::AddressKind::V4,
          ).await {
      ...
      memberlist_core::unit_tests_with_expr!($run(
        [< $($prefix)? _handle_v4_ping_server_no_label_with_compression_client_no_label_no_compression >] ({
          let s = $s;
          let c = $s;
          if let Err(e) = memberlist_quic::tests::handle_ping::no_label_with_compression::server_no_label_with_compression_client_no_label_no_compression::<$layer<$rt>, $rt>(
            s,
            c,
            memberlist_core::transport::tests::AddressKind::V4,
          ).await {
