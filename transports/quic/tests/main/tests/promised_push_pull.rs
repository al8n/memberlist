    ...
    paste::paste! {
      memberlist_core::unit_tests_with_expr!($run(
        ...
        [< $($prefix:snake)? _v4_promised_push_pull >] ({
          ...
          if let Err(e) = memberlist_quic::tests::promised_push_pull::promised_push_pull::<_, $rt>(s, client, kind).await {
            panic!("{}", e);
          }
        }),
        #[cfg(all(feature = "encryption", feature = "compression"))]
      ...
      memberlist_core::unit_tests_with_expr!($run(
        ...
        [< $($prefix:snake)? _v4_promised_push_pull_no_label >] ({
          ...
          let (_, ln, connector) = memberlist_quic::stream_layer::StreamLayer::bind(&c, client_addr).await.unwrap();
          let client = memberlist_quic::tests::QuicTransportTestPromisedClient::new(c, ln, connector);
          if let Err(e) = memberlist_quic::tests::promised_push_pull::promised_push_pull_no_label::<_, $rt>(s, client, kind).await {
            panic!("{}", e);
          }
        }),
        #[cfg(all(feature = "encryption", feature = "compression"))]
      ...
      memberlist_core::unit_tests_with_expr!($run(
        ...
        [< $($prefix:snake)? _v4_promised_push_pull_encryption_only >] ({
          ...
          let (_, ln, connector) = memberlist_quic::stream_layer::StreamLayer::bind(&c, client_addr).await.unwrap();
          let client = memberlist_quic::tests::QuicTransportTestPromisedClient::new(c, ln, connector);
          if let Err(e) = memberlist_quic::tests::promised_push_pull::promised_push_pull_encryption_only::<_, $rt>(s, client, kind).await {
            panic!("{}", e);
          }
        }),
        #[cfg(feature = "encryption")]
      ...
      memberlist_core::unit_tests_with_expr!($run(
        ...
        [< $($prefix:snake)? _v4_promised_push_pull_label_and_encryption >] ({
          ...
          let client = memberlist_quic::tests::QuicTransportTestPromisedClient::new(c, ln, connector);
          if let Err(e) =
          memberlist_quic::tests::promised_push_pull::promised_push_pull_label_and_encryption::<_, $rt>(s, client, kind).await
          {
            panic!("{}", e);
          }
        }),
