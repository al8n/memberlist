...
handle_compound_ping_test_suites!("tcp": Tcp<AsyncStdRuntime>::run({
  ()
}));

#[cfg(feature = "tls")]
handle_compound_ping_test_suites!("tls": Tls<AsyncStdRuntime>::run({
  memberlist_net::tests::tls_stream_layer::<AsyncStdRuntime>().await
}));

#[cfg(feature = "native-tls")]
handle_compound_ping_test_suites!("native_tls": NativeTls<AsyncStdRuntime>::run({
