...
handle_compound_ping_test_suites!("tcp": Tcp<TokioRuntime>::run({
  ()
}));

#[cfg(feature = "tls")]
handle_compound_ping_test_suites!("tls": Tls<TokioRuntime>::run({
  memberlist_net::tests::tls_stream_layer::<TokioRuntime>().await
}));

#[cfg(feature = "native-tls")]
handle_compound_ping_test_suites!("native_tls": NativeTls<TokioRuntime>::run({
