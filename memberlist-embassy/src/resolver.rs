//! Asynchronous address resolution: the [`AddressResolver`] trait and the trivial
//! [`SocketAddrResolver`].
//!
...
/// Asynchronously resolves an unresolved address (e.g. a `host:port` name) into
/// candidate wire [`SocketAddr`]s.
///
/// Async to match embassy's async runtime, but free of `Send`/`Sync`/`'static`
/// bounds: the embassy driver runs on a single executor and shares its state by
/// [`Rc`](alloc::rc::Rc), so a resolver is never moved across threads. The
/// resolver returns zero or more candidates, since one name may map to several
/// A/AAAA records.
///
...
/// result for the driver to allocate and truncate after the fact — there is no
/// caller `limit` to pass or honor.
pub trait AddressResolver {
  /// The unresolved address this resolver accepts.
  type Address;

  /// The error returned when resolution fails. `'static` so the driver can box
  /// it into a typed [`InitError`](crate::InitError) / [`OpError`](crate::OpError)
  ...
  ) -> impl core::future::Future<Output = Result<ResolvedAddrs, Self::Error>>;
}
